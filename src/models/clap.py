...
class AudioEncoder(nn.Module):
    ...
    def forward(self, x, is_spec_augment=False, mixup_lambda=None, mixup_perm=None):
        ...
        audio_features, audio_classification_output, frame_features = out_dict['embedding'], out_dict['clipwise_output'], out_dict['frame_embedding']
        projected_vec = self.projection(audio_features)
        projected_frame_vec = self.projection(frame_features)
        return projected_vec, audio_classification_output, projected_frame_vec
...
class TextEncoder(nn.Module):
    ...
    def forward(self, x, use_prompt=False, prompts=None, prompt_attention_mask=None, return_word=False):
        if not use_prompt:
            if not return_word:
                out = self.base(**x)[0]
                ...
                out = self.base(**x)[0]
                out = out[:, 1:, :]  # get CLS token output
                projected_vec = self.projection(out)
                return projected_vec
        else:           
...
class CLAP(nn.Module):
    ...
    def encode_audio(self, audio, is_spec_augment=False, mixup_lambda=None, mixup_perm=None):
        audio_embed, _, _ = self.audio_encoder(audio, is_spec_augment=is_spec_augment, mixup_lambda=mixup_lambda, mixup_perm=mixup_perm)
        return audio_embed

    def encode_frame(self, audio, is_spec_augment=False, mixup_lambda=None, mixup_perm=None):
        _, _, frame_embed = self.audio_encoder(audio, is_spec_augment=is_spec_augment, mixup_lambda=mixup_lambda, mixup_perm=mixup_perm)
        return frame_embed
